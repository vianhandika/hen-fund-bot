...
const WS_PRIVATE_URL = String(ENV.BYBIT_WS_PRIVATE_URL ?? '').trim();
...
export const subscribeFills = async (
  ...
  ws.on('open', () => {
    logger.info(
      { url: WS_PRIVATE_URL, key: maskKey(API_KEY) },
      'WS private: open'
    );
  ...
  ws.on('message', (raw) => {
    try {
      ...
      if (msg.op === 'auth') {
        ...
        logger.info(
          { success: ok, ret_msg: retMsg },
          'WS private: auth result'
        );
      ...
      if (authed && msg.topic === 'execution' && Array.isArray(msg.data)) {
        for (const ex of msg.data) {
          ...
          const tag = link
            ? String(link).split('-')[0]?.toUpperCase() || ''
            : '';
