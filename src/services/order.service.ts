...
// === konstanta strategi ===
import { TP1_PCT, TP2_PCT, TP3_PCT, TRIM_PCTS } from '../config/constants.js';
...
export const placeDCAOrders = async (
  ...
  for (const l of legs) {
    ...
    const sellMin = (band as any).sellMin ?? (last ? last * 0.8 : price);
...
/** Hitung harga TP berdasarkan avg sesuai konstanta SHORT strategy (3 TP) */
export const buildShortTPPrices = (avg: number) => {
  const tp1 = avg * (1 - TP1_PCT);
  const tp2 = avg * (1 - TP2_PCT);
  const tp3 = avg * (1 - TP3_PCT);
  return [tp1, tp2, tp3];
};

/** Tunggu posisi muncul terlebih dahulu di Bybit (kadang butuh ~200–500ms) */
export const waitUntilPositionVisible = async (
  symbol: string,
  timeoutMs = 6000
) => {
...
/** Pasang TP reduce-only (RO) sesuai TRIM_PCTS — 3 TP */
export const placePositionTPs = async (
  ...
  const prices = buildShortTPPrices(avgPrice); // 3 harga TP
  ...
  for (let i = 0; i < legs.length; i++) {
    const raw = legs[i];
    if (!raw.qty) continue; // skip kalau 0
    ...
    try {
      ...
      logger.error(
        { symbol, tag, price: p, qty: q, err: e?.message || e },
        'RO TP submit failed'
      );
    }
  }
